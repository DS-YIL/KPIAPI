...
import { userDetails } from './monthlykpistatus.model';

@Injectable({
    providedIn: 'root'
  })
  export class monthlykpistatusComponentService {
    ...
    getmonthlyKPIstatus(employee:Employee): Observable<any> {
        const httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json' ,'Timeout' : '120000'}) };
        return this.http.post<any>(this.url + 'KPI/getmonthlyKPIstatus',employee, httpOptions);
      }
